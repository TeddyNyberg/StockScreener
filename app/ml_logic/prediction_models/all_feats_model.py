...
from app.data.feature_engineer import feat_engr
from app.data.preprocessor_utils import to_seq, normalize_window, normalize_multivariate_window, to_multi_seq
...
def train_fn(args):
    ...
    list_of_dfs = feat_engr(list_of_dfs)

    feature_cols = [c for c in list_of_dfs[0].columns if c not in ['Date', 'Symbol']]

    price_vars = ['Close', 'MA_50', 'BB_Upper', 'BB_Lower', 'High', 'Low', 'Open']
    norm_indices = [i for i, col in enumerate(feature_cols) if col in price_vars]

    close_idx = feature_cols.index("Close")

    num_features = len(feature_cols)
    print(f"Training on {num_features} features.")
    print(f"Z-Scoring indices: {norm_indices} ({[feature_cols[i] for i in norm_indices]})")
    ...
    for df in list_of_dfs:
        # CRITICAL FIX: Keep ALL feature columns, not just Close
        data_arr = df[feature_cols].values.astype(np.float32)

        split_idx = int(len(data_arr) * 0.8)
        list_close_train.append(data_arr[:split_idx])
        list_close_test.append(data_arr[split_idx:])


    norm_windows, targets = [], []
    for arr in list_close_train:
        xt, yt = to_multi_seq(SEQUENCE_SIZE, arr, close_idx)
        for x_win, y_val in zip(xt, yt):
            x_norm, y_norm, _, _ = normalize_multivariate_window(
                x_win, y_val, close_idx, norm_indices)
            norm_windows.append(x_norm)
            targets.append(y_norm)
